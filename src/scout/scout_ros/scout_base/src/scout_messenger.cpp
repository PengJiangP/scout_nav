...
//      bms_status.Warning_Status_2 = state.Warning_Status_2;
                                               ...
                                               double dt)
  {
    ...
    // 设置默认的不可观测状态（Z, Roll, Pitch）为非常大的不确定性，避免EKF报错或漂移
    // 1e6 表示极度不确定，EKF会忽略这些维度
    ...
    // 定义运动阈值
    double vel_tol = 0.02; // 认为是静止的速度阈值 (m/s)
    double w_tol = 0.02;   // 认为是静止的角速度阈值 (rad/s)
    double turn_thresh = 0.15; // 认为是正在转弯的角速度阈值 (rad/s)

    // 定义协方差变量
    double cov_pose_xy, cov_pose_yaw;
    double cov_twist_x, cov_twist_y, cov_twist_yaw;

    // --- 分段逻辑判断 ---
    if (std::abs(linear) < vel_tol && std::abs(angular) < w_tol) {
        // 【状态 1：静止】
        // 机器人完全不动，轮速计非常可信（速度为0）
        
        // Twist (速度) 协方差：极小，表示我们非常确信速度是0
        cov_twist_x = 1e-9; 
        cov_twist_y = 1e-9; 
        cov_twist_yaw = 1e-9;

        // Pose (位置) 协方差：保持一个较小值，表示位置稳定
        cov_pose_xy = 1e-3; 
        cov_pose_yaw = 1e-3;
    } 
    else if (std::abs(angular) > turn_thresh) {
        // 【状态 2：转弯】
        // 差速车转弯时打滑严重，轮速计极不可信，尤其是 Yaw 和 Y 方向
        
        // Twist (速度) 协方差：设大，告诉 EKF "我不准，别信我，去信雷达/IMU"
        cov_twist_x = 0.5;   // x方向也会因为打滑而不准
        cov_twist_y = 0.5;   // 侧向打滑严重
        cov_twist_yaw = 0.5; // 角度极不准
        ...
        // Twist (速度) 协方差：正常值
        cov_twist_x = 0.05;  // 直线编码器还算准
        cov_twist_y = 0.05;  // 侧向应该没什么滑移
        cov_twist_yaw = 0.05; // 只有轻微角度修正
    ...
    // 填充对角线
    odom_msg.pose.covariance[0] = cov_pose_xy;   // X
    odom_msg.pose.covariance[7] = cov_pose_xy;   // Y
    odom_msg.pose.covariance[35] = cov_pose_yaw; // Yaw
    odom_msg.twist.covariance[0] = cov_twist_x;   // Vx
    odom_msg.twist.covariance[7] = cov_twist_y;   // Vy
    odom_msg.twist.covariance[35] = cov_twist_yaw; // Vyaw
