...
bool CLaserOdometry2DNode::setLaserPoseFromTf()
{
  ...
  try
  {
    tf_listener.waitForTransform("/base_link","/laser_link", ros::Time(), ros::Duration(5.0));
...
void CLaserOdometry2DNode::publish()
{
  ...
  // 新增：填充协方差矩阵（关键！必须在 odom 定义之后）
  // --------------------------
  // 5.1 填充 pose.covariance（6x6，对应 x,y,z,rx,ry,rz）
  double scale_factor = 10000.0;
  odom.pose.covariance.assign(0.0); // 先初始化全 0（assign 是 vector 的成员函数）
  // 从 cov_odo（3x3，x/y/yaw 协方差）映射到 6x6 矩阵
  odom.pose.covariance[0]  = cov_odo(0,0) * scale_factor;  // x-x 方差
  odom.pose.covariance[1]  = cov_odo(0,1) * scale_factor;  // x-y 协方差
  odom.pose.covariance[5]  = cov_odo(0,2) * scale_factor;  // x-rz 协方差（rz 是偏航角）
  odom.pose.covariance[7]  = cov_odo(1,1) * scale_factor;  // y-y 方差
  odom.pose.covariance[11] = cov_odo(1,2) * scale_factor;  // y-rz 协方差
  odom.pose.covariance[35] = cov_odo(2,2) * scale_factor;  // rz-rz 方差（偏航角方差）

  // 5.2 填充 twist.covariance（6x6，对应 vx,vy,vz,wx,wy,wz）
  odom.twist.covariance.assign(0.0); // 初始化全 0
  // 速度协方差基于位置协方差和帧率推导（freq 是节点参数，单位 Hz）
  odom.twist.covariance[0]  = cov_odo(0,0) * freq * freq * scale_factor;  // vx-vx 方差（vx = dx * freq）
  odom.twist.covariance[1]  = cov_odo(0,1) * freq * freq * scale_factor;  // vx-vy 协方差
  odom.twist.covariance[5]  = cov_odo(0,2) * freq * scale_factor;         // vx-wz 协方差（wz = d(rz) * freq）
  odom.twist.covariance[7]  = cov_odo(1,1) * freq * freq * scale_factor; // vy-vy 方差
  odom.twist.covariance[11] = cov_odo(1,2) * freq * scale_factor;         // vy-wz 协方差
  odom.twist.covariance[35] = cov_odo(2,2) * freq * freq * scale_factor;  // wz-wz 方差
  //publish the message
